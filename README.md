# Fordewind

Веб-приложение на Laravel для попарного голосования за фотографии аукционных автомобилей и просмотра статистики голосования.

## Задача

Необходимо реализовать две независимые клиентские области:

- **Голосование:** пользователь выбирает модель автомобиля, получает две разные фотографии и голосует за одну из них. Цикл начинается со случайной точки и показывает каждую новую фотографию до завершения; при нечётном количестве автомобилей последняя новая фотография выводится в паре с уже показанной. Пары загружаются через jQuery/AJAX без перезагрузки страницы, а фотографии увеличиваются с помощью jQuery ezPlus в режиме Tints.
- **Статистика:** Vue.js отображает автомобили и число голосов, позволяет фильтровать результаты по модели и диапазону годов выпуска, а также показывает суммарное число голосов для выбранных фильтров.


## Стек

- PHP 8.3 и Laravel 12;
- MySQL 8.4 для разработки и отдельная MySQL БД для PHPUnit;
- Nginx и PHP-FPM в Docker Compose;
- Vue 3 для статистики;
- jQuery 3 и ezPlus для страницы голосования;
- Vite 7 для frontend-сборки;
- Task для единых локальных команд.

## Требования

- Docker Engine с Docker Compose v2;
- Для быстрого запуска нужно установить [Task](https://taskfile.dev/);
- свободный порт по умолчанию `8082` для приложения и `33063` для MySQL.

## Быстрый старт

```bash
cp .env.example .env
task init
```

`task init` создаёт runtime-директории, поднимает контейнеры, устанавливает Composer/npm-зависимости, генерирует `APP_KEY`, применяет миграции, клонирует и импортирует исходные автомобили с JPEG-изображениями, создаёт `storage` symlink и собирает frontend. Для этого нужен доступ к `CARS_IMPORT_REPOSITORY`.

После успешной инициализации приложение доступно по адресу [http://localhost:8082](http://localhost:8082).

Справочник доступных Task команд: [docs/commands.md](docs/commands.md).

## Локальная конфигурация

В `.env` можно изменить:

- `APP_PORT` - внешний HTTP-порт, по умолчанию `8082`;
- `FORWARD_DB_PORT` - внешний порт dev MySQL, по умолчанию `33063`;
- `USER_ID` и `GROUP_ID` - UID/GID пользователя хоста. Значения по умолчанию `1000` подходят для стандартного Linux-окружения;
- `DB_*` - подключение к основной dev БД;
- `DB_TEST_*` - подключение к изолированной БД тестов.
- `CARS_IMPORT_REPOSITORY` - URL Git-репозитория с исходными JSON;
- `CARS_IMPORT_SOURCE_PATH` - каталог, в который `task import` клонирует репозиторий и из которого читает JSON.
- `CARS_IMPORT_BATCH_SIZE` - размер чанка импорта, по умолчанию `200`;
- `CARS_IMPORT_WORK_PATH` - временный каталог файловых маркеров дедупликации `AuctionItemId` в рамках запуска импорта;
- `CARS_IMPORT_IMAGES_PATH` - каталог JPG-изображений в источнике; если не задан, используется `CARS_IMPORT_SOURCE_PATH`;
- `CARS_PUBLIC_IMAGES_PATH` - каталог назначения изображений в `public`;
- `VOTING_VOTES_*` и `VOTING_PAIRS_*` - количество запросов и длина окон rate limit для голосования и получения пар; полный набор и значения по умолчанию перечислены в `.env.example`.

Не добавляйте `.env` в Git. Шаблон содержит только локальные значения разработки без реальных секретов.

## Сервисы Docker

| Сервис | Назначение |
| --- | --- |
| `nginx` | HTTP-сервер на `APP_PORT` |
| `app` | PHP 8.3 FPM, Artisan и Composer |
| `mysql` | Основная dev MySQL БД |
| `mysql-test` | Изолированная MySQL БД для PHPUnit |
| `frontend` | Node 22 для npm, Vite и frontend-сборки |


## Frontend

Vue 3 подключён через `@vitejs/plugin-vue`. Отдельные entry points размещаются в `resources/js`:

- [/vote](/vote) и главная страница используют `resources/js/vote.js`: jQuery загружает модели и пары, отправляет голос с CSRF-токеном, а ezPlus увеличивает фотографии в режиме Tints;
- [/statistics](/statistics) использует Vue-компонент `resources/js/components/StatisticsApp.vue`: фильтры, debounce для годов, карточки, общий счётчик и пагинация обновляются через JSON API без перезагрузки;
- по окончании цикла на странице голосования клиент вызывает `POST /api/voting/cycle`, получает новую session и новый CSRF-токен.

Сборка frontend проверяется командой `task build`.

## JSON API

API размещён в `web` middleware под префиксом `/api`: это сохраняет анонимную Laravel-session для цикла голосования и стандартную CSRF-защиту для `POST`-запроса.

| Метод и путь | Назначение |
| --- | --- |
| `GET /api/voting/models` | Доступные модели и количество автомобилей каждой модели |
| `GET /api/voting/pair?make={make}&model={model}` | Следующая пара для марки и модели; ответ содержит `status: ready`, `unavailable` или `exhausted` |
| `POST /api/voting/cycle` | Начинает новый цикл в новой Laravel-session и возвращает новый `csrf_token` |
| `POST /api/voting/votes` | Сохраняет голос текущей пары и возвращает `nextPair` |
| `GET /api/statistics` | Статистика с `model`, `year_from`, `year_to`, `page` и `per_page` |

Ответ готовой пары содержит одноразовый `pair_token`. Для сохранения голоса передаются `make`, `model`, `left_car_id`, `right_car_id`, `winner_side` (`left` или `right`) и этот `pair_token`. Сервер принимает только пару, ранее выданную той же session, не допускает повторной отправки токена и повторного выбора одной пары в пределах анонимной сессии. В БД сохраняется HMAC session ID, а не исходный идентификатор сессии. Частота отправки ограничена по session и IP; без авторизации это базовая защита, а не абсолютная защита от пользователя, который может менять IP и cookies. Все JSON-поля используют `snake_case`: например, ответ голоса содержит `next_pair`, а статистика - `votes_received` и `total_votes`. Ошибки API имеют единый вид: `message` и `errors`.

## Использование ИИ

История фактически использованных запросов ИИ ведётся единым списком в [docs/ai-artifacts/prompt-history.md](docs/ai-artifacts/prompt-history.md).

Настройки и инструкции агента: [AGENTS.md](AGENTS.md). Применяемые project-specific skills: [.agents/README.md](.agents/README.md).
